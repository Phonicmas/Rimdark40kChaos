...
namespace Chaos40k
{
    public class Need_KhornateHunger : Need
    {
        ...
        public override void NeedInterval()
        {
            ...
            if (CurLevel > 0 && pawn.MentalStateDef == Chaos40kDefOf.BEWH_KhornateHungerBeserk)
            {
                pawn.MentalState.RecoverFromState();
            }
            if (CurLevel == 0 && !pawn.InMentalState)
            {
                pawn.mindState.mentalStateHandler.TryStartMentalState(Chaos40kDefOf.BEWH_KhornateHungerBeserk);
            }
        }
